...
public:
	...
	*/
	Texture(UnsignedImage* image, const GLuint wrapS = WRAP_S, const GLuint wrapT = WRAP_T, const GLuint minFilter = MIN_FILTER, const GLuint magFilter = MAG_FILTER);

	/**
	*	@brief Constructor from an image given by a float array. 
	*/
	Texture(float* image, const int width, const int height, const bool isRGBA = true, const GLuint wrapS = WRAP_S, const GLuint wrapT = WRAP_T, 
	...
	*/
	Texture(std::vector<UnsignedImage*> images, const int width, const int height, const GLuint wrapS = WRAP_S, const GLuint wrapT = WRAP_T, const GLuint wrapR = WRAP_R, 
			const GLuint minFilter = MIN_FILTER, const GLuint magFilter = MAG_FILTER);
