...
Group3D::Group3D(const mat4& modelMatrix):
	Model3D(modelMatrix), _staticGPUData(nullptr)
{
}
...
Group3D::StaticGPUData* Group3D::generateBVH()
{
	...
	ComputeShader* findNeighborShader		= ShaderList::getInstance()->getComputeShader(CGEnumerations::FIND_BEST_NEIGHBOR);
	ComputeShader* clusterMergingShader		= ShaderList::getInstance()->getComputeShader(CGEnumerations::CLUSTER_MERGING);
	ComputeShader* reallocClustersShader	= ShaderList::getInstance()->getComputeShader(CGEnumerations::REALLOCATE_CLUSTERS);
	ComputeShader* endLoopCompShader		= ShaderList::getInstance()->getComputeShader(CGEnumerations::END_LOOP_COMPUTATIONS);

	// Prefix scan
	ComputeShader* reduceShader				= ShaderList::getInstance()->getComputeShader(CGEnumerations::REDUCE_PREFIX_SCAN);
	ComputeShader* downSweepShader			= ShaderList::getInstance()->getComputeShader(CGEnumerations::DOWN_SWEEP_PREFIX_SCAN);
...
void Group3D::aggregateSSBOData(VolatileGPUData*& volatileGPUData, StaticGPUData*& staticGPUData)
{
	...
	size_t currentGeometry = 0, currentTopology = 0;
...
{
	ComputeShader* buildClusterShader = ShaderList::getInstance()->getComputeShader(CGEnumerations::BUILD_CLUSTER_BUFFER);
...
{
	ComputeShader* computeAABBShader	= ShaderList::getInstance()->getComputeShader(CGEnumerations::COMPUTE_GROUP_AABB);
...
{
	ComputeShader* computeMortonShader = ShaderList::getInstance()->getComputeShader(CGEnumerations::COMPUTE_MORTON_CODES);
...
{
	ComputeShader* bitMaskShader			= ShaderList::getInstance()->getComputeShader(CGEnumerations::BIT_MASK_RADIX_SORT);
	ComputeShader* reduceShader				= ShaderList::getInstance()->getComputeShader(CGEnumerations::REDUCE_PREFIX_SCAN);
	ComputeShader* downSweepShader			= ShaderList::getInstance()->getComputeShader(CGEnumerations::DOWN_SWEEP_PREFIX_SCAN);
	ComputeShader* resetPositionShader		= ShaderList::getInstance()->getComputeShader(CGEnumerations::RESET_LAST_POSITION_PREFIX_SCAN);
